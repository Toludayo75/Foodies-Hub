...
import { ChevronLeft, ArrowRight, ShoppingBag, Bell, Info, Check } from "lucide-react";
...
export default function Notifications() {
  ...
  const { data: notifications = [], isLoading: loadingNotifications, refetch } = useQuery<Notification[]>({
          ...
          <TabsContent value="all" className="mt-4 space-y-3">
            {notifications.length === 0 ? (
              ...
              notifications.map(notification => (
                <div 
                  key={notification.id}
                  className={`bg-white rounded-lg p-4 shadow-sm relative flex ${!notification.isRead ? 'border-l-4 border-foodies-green' : ''}`}
            ...
            {unreadCount === 0 ? (
              ...
              notifications
                ...
                .map(notification => (
                  <div 
                    key={notification.id}
                    className="bg-white rounded-lg p-4 shadow-sm relative flex border-l-4 border-foodies-green"
                    onClick={() => handleNotificationClick(notification.id)}
                  >
                    <div className="mr-3 mt-1">
                      {getNotificationIcon(notification.type)}
